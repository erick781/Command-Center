...
export type WorkflowTaskFamilyDefinition = {
  description: string;
  id: WorkflowTaskFamily;
  label: string;
};

export const workflowTaskFamilies: WorkflowTaskFamilyDefinition[] = [
  {
    id: "strategy",
    label: "Strategy",
    description: "Plan and direction.",
  },
  {
    id: "summary",
    label: "Summary",
    description: "Recap and handoff.",
  },
  {
    id: "reporting",
    label: "Reporting",
    description: "Performance and next steps.",
  },
  {
    id: "creative",
    label: "Creative",
    description: "Angles and briefs.",
  },
  {
    id: "copywriting",
    label: "Copywriting",
    description: "Ads, SMS, email, pages.",
  },
  {
    id: "email",
    label: "Email",
  ...
  },
  {
    id: "content",
    label: "Content",
    description: "Ideas and hooks.",
  },
];

export const workflowTasks: WorkflowTaskDefinition[] = [
  {
    description: "Build a strategic plan.",
    family: "strategy",
    icon: "target",
    id: "strategy_360",
    label: "Strategy 360",
    ...
    summary: "Growth plan from client context.",
  },
  {
    description: "Create a clean recap.",
    family: "summary",
    icon: "fileText",
    id: "client_summary",
    ...
    summary: "Decisions, risks, next steps.",
  },
  {
    ...
    summary: "Signal-based report and actions.",
  },
  {
    description: "Sharpen the creative brief.",
    family: "creative",
    icon: "palette",
    id: "creative_brief",
    ...
    summary: "Direction, hooks, formats.",
  },
  {
    ...
    summary: "Hooks, body, CTA.",
  },
  {
    description: "Write an SMS sequence.",
    family: "copywriting",
    icon: "messageSquare",
    id: "sms_sequence",
    label: "SMS Sequence",
    outputLabel: "SMS sequence",
    promptFlavor: "copy",
    questionCap: 2,
    questions: [
      {
        id: "goal",
        label: "Goal",
        placeholder: "What should this SMS sequence drive right now?",
        priority: 1,
        required: true,
        rows: 2,
        type: "textarea",
      },
      {
        id: "offer",
        label: "Offer / CTA",
        placeholder: "What are people being asked to do or claim?",
        priority: 2,
        rows: 2,
        type: "textarea",
      },
      ...
      },
      {
        ...
        placeholder: "Promo, deadline, launch, same-day push, follow-up window, etc.",
        priority: 4,
        rows: 2,
        type: "textarea",
      },
    ],
    summary: "Short, clear, action-focused SMS.",
  },
  {
    ...
    summary: "Angles, subjects, sequence.",
  },
  {
    ...
    questions: [
      {
        id: "page_goal",
        label: "Page goal",
        placeholder: "Lead form, booking, quote request, purchase, application, etc.",
        priority: 1,
        required: true,
        rows: 2,
        type: "textarea",
      },
      {
        id: "offer",
        label: "Offer / promise",
        placeholder: "What transformation, package, or promise is the page centered on?",
        priority: 2,
        rows: 3,
        type: "textarea",
      },
      {
        id: "cta",
        label: "Primary CTA",
        placeholder: "Book a call, request a quote, get the audit, apply now, etc.",
        priority: 3,
        rows: 2,
        type: "textarea",
      },
      {
        id: "proof",
        label: "Proof or credibility angle",
        placeholder: "Results, testimonials, guarantees, process, case studies, etc.",
        priority: 4,
        rows: 3,
        type: "textarea",
      },
    ],
    summary: "Hero, sections, proof, CTA.",
  },
  {
    ...
    summary: "Hooks, angles, post ideas.",
  },
];
  ...
  switch (taskId) {
    ...
    case "sms_sequence":
      return [
        !signals.hasManualContext ? "goal" : null,
        !signals.hasOffer ? "offer" : null,
        !signals.hasManualContext && !signals.hasStrategyMemory ? "timing" : null,
      ];
    case "email_sequence":
      return [
        !signals.hasManualContext ? "goal" : null,
    ...
    case "landing_page":
      return [
        "page_goal",
        !signals.hasOffer ? "offer" : null,
        !signals.hasManualContext && !signals.hasAudience ? "cta" : null,
        !signals.hasCreativeHistory ? "proof" : null,
      ];
    case "content_ideas":
      return [
...
function buildTaskProfileNeeds(taskId: string, signals: WorkflowTaskSignals, language: CommandCenterLanguage) {
  const shared = [
    ...
    !signals.hasTone && (taskId === "ad_copy" || taskId === "email_sequence" || taskId === "content_ideas")
      ...
      : null,
    !signals.hasFunnelContext && (taskId === "landing_page" || taskId === "email_sequence" || taskId === "sms_sequence")
      ? t(language, "Complète le sales process ou le follow-up process pour cadrer le bon CTA et la bonne suite.", "Fill in the sales process or follow-up process to frame the right CTA and next step.")
      : null,
...
function buildTaskWarnings(taskId: string, signals: WorkflowTaskSignals, language: CommandCenterLanguage, questionCount: number) {
  const warnings = [
    ...
    taskId === "performance_report" && !signals.hasPerformanceSnapshot
      ...
      : null,
    (taskId === "ad_copy" || taskId === "email_sequence" || taskId === "sms_sequence" || taskId === "landing_page") && !signals.hasOffer
...
export function resolveWorkflowTask(
  ...
  if (task.id === "sms_sequence") {
    defaultAnswers.sms_count = defaultAnswers.sms_count || "3";
  }
...
export function getWorkflowTasksByFamily(family: WorkflowTaskFamily) {
  return workflowTasks.filter((task) => task.family === family);
}
...
const frWorkflowTaskTranslations: Record<string, WorkflowTaskTranslation> = {
  strategy_360: {
    description: "Construire le plan.",
    label: "Stratégie 360",
    outputLabel: "Stratégie de croissance",
    summary: "Plan de croissance clair.",
    questions: {
      objective: {
  ...
  client_summary: {
    description: "Créer le récap.",
    label: "Résumé client",
    outputLabel: "Résumé client",
    summary: "Décisions, risques, actions.",
    questions: {
      meeting_notes: {
  ...
  performance_report: {
    description: "Lire la performance.",
    label: "Rapport de performance",
    outputLabel: "Rapport de performance",
    summary: "Signaux, constats, actions.",
    questions: {
      report_type: {
  ...
  sms_sequence: {
    description: "Rédiger les SMS.",
    label: "Séquence SMS",
    outputLabel: "Séquence SMS",
    summary: "Court, clair, action.",
    questions: {
      goal: {
        label: "Objectif",
        placeholder: "Quel résultat cette séquence SMS doit-elle générer maintenant ?",
      },
      offer: {
        label: "Offre / CTA",
        placeholder: "Qu’est-ce qu’on demande précisément aux gens de faire ou de réclamer ?",
      },
      sms_count: {
        label: "Nombre de SMS",
        options: [
      ...
      timing: {
        label: "Timing ou urgence",
        placeholder: "Promo, deadline, lancement, relance, fenêtre d’action, etc.",
      },
    },
  },
  email_sequence: {
    description: "Rédiger les emails.",
    label: "Séquence email",
    outputLabel: "Séquence email",
    summary: "Angles, sujets, structure.",
    questions: {
      sequence_type: {
  ...
  landing_page: {
    description: "Rédiger la landing.",
    label: "Landing page",
    outputLabel: "Landing page",
    summary: "Hero, sections, preuves, CTA.",
    questions: {
      page_goal: {
        label: "Objectif de la page",
        placeholder: "Lead form, booking, quote request, purchase, application, etc.",
      },
      offer: {
        label: "Offre / promesse",
        placeholder: "Quelle transformation, package ou promesse doit porter la page ?",
      },
      cta: {
        label: "CTA principal",
        placeholder: "Book a call, request a quote, get the audit, apply now, etc.",
      ...
      proof: {
        label: "Preuve ou crédibilité",
        placeholder: "Résultats, témoignages, garanties, process, case studies, etc.",
      },
    },
  },
  content_ideas: {
    description: "Trouver les idées.",
    label: "Idées de contenu",
    outputLabel: "Set d’idées de contenu",
    summary: "Hooks, angles, idées.",
    questions: {
      platforms: {
        label: "Plateformes",
...
const frWorkflowTaskFamilyTranslations: Record<WorkflowTaskFamily, Partial<WorkflowTaskFamilyDefinition>> = {
  strategy: {
    label: "Stratégie",
    description: "Plan et direction.",
  },
  summary: {
    label: "Résumé",
    description: "Récap et handoff.",
  },
  reporting: {
    label: "Rapport",
    description: "Performance et actions.",
  },
  creative: {
    label: "Créatif",
    description: "Angles et briefs.",
  },
  copywriting: {
    label: "Rédaction",
    description: "Pubs, SMS, email, pages.",
  },
  email: {
    label: "Email",
    description: "Lifecycle messaging.",
  ...
  content: {
    label: "Contenu",
    description: "Idées et hooks.",
  },
};

export function localizeWorkflowTaskFamily(
  family: WorkflowTaskFamilyDefinition,
  language: CommandCenterLanguage,
): WorkflowTaskFamilyDefinition {
  if (language !== "fr") {
    return family;
  }

  const translation = frWorkflowTaskFamilyTranslations[family.id];
